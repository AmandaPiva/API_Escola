...
CREATE TABLE ESC_PROFESSOR(
	...
	PRO_CH_CPF CHAR(20) NOT NULL CONSTRAINT PRO_CK_CH_CPF CHECK (PRO_CH_CPF LIKE
...
CREATE TABLE ESC_ALUNO(
	...
	ALU_CH_CPF CHAR(20) NOT NULL CONSTRAINT ALU_CK_CH_CPF CHECK (ALU_CH_CPF LIKE 
	'[0-9][0-9][0-9][.][0-9][0-9][0-9][.][0-9][0-9][0-9][-][0-9][0-9]')
...
(@ra int, @nome varchar(100), @cpf char(20))
AS

DECLARE @ID_ALUNO int

SELECT @ID_ALUNO = COUNT(*) FROM ESC_ALUNO WHERE ALU_IN_RA = @ra;

--VERIFICANDO SE JÁ EXISTE NO BANCO DE DADOS
IF(@ID_ALUNO > 0)
	BEGIN 

	RAISERROR('O ra já existe no BD', 15,1)
	RETURN
	END
...
--EXECUTANDO O INSERT
INSERT INTO ESC_ALUNO (ALU_IN_RA, ALU_ST_NOME, ALU_CH_CPF)
VALUES (@ra, @nome, @cpf)

GO --FIM DA PROCEDURE

--testando 
EXEC SP_I_ESC_ALUNO 'Rafael Matos', '516.492.728-47'
...
(@rf int, @nome varchar(100), @cpf char(20), @especialidade varchar(100))
AS

DECLARE @ID_PROFESSOR int

SELECT @ID_PROFESSOR = COUNT(*) FROM ESC_PROFESSOR WHERE PRO_IN_RF = @rf;

--VERIFICANDO SE JÁ EXISTE NO BANCO DE DADOS
IF(@ID_PROFESSOR > 0)
	BEGIN 

	RAISERROR('O rf já existe no BD', 15,1)
	RETURN
	END
...
--EXECUTANDO O INSERT
INSERT INTO ESC_PROFESSOR(PRO_IN_RF, PRO_ST_NOME, PRO_CH_CPF, PRO_ST_ESPECIALIDADE)
VALUES (@rf, @nome, @cpf, @especialidade);

GO --FIM DA PROCEDURE

EXEC SP_I_ESC_PROFESSOR 'Nathalia Buso', '516.492.728-47', 'Banco de Dados'
...
CREATE PROCEDURE SP_UP_ESC_ALUNO
...
EXEC SP_UP_ESC_PROFE 3, 'Alice Campos', '444.555.889-74,', 'Administração'
